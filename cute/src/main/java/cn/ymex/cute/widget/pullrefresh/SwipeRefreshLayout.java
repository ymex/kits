 ...
 *
 * from: https://github.com/Aspsine/SwipeToLoadLayout
 */
public class SwipeRefreshLayout extends ViewGroup {
    ...
    private OnPushRefreshListener mPushRefreshListener;
    ...
    public SwipeRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SwipeRefreshLayout, defStyleAttr, 0);
        try {
            final int N = a.getIndexCount();
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == R.styleable.SwipeRefreshLayout_refresh_enabled) {
                    setRefreshEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.SwipeRefreshLayout_load_more_enabled) {
                    setLoadMoreEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.SwipeRefreshLayout_swipe_style) {
                    setSwipeStyle(a.getInt(attr, STYLE.CLASSIC));

                } else if (attr == R.styleable.SwipeRefreshLayout_drag_ratio) {
                    setDragRatio(a.getFloat(attr, DEFAULT_DRAG_RATIO));

                } else if (attr == R.styleable.SwipeRefreshLayout_refresh_final_drag_offset) {
                    setRefreshFinalDragOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.SwipeRefreshLayout_load_more_final_drag_offset) {
                    setLoadMoreFinalDragOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.SwipeRefreshLayout_refresh_trigger_offset) {
                    setRefreshTriggerOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.SwipeRefreshLayout_load_more_trigger_offset) {
                    setLoadMoreTriggerOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.SwipeRefreshLayout_swiping_to_refresh_to_default_scrolling_duration) {
                    setSwipingToRefreshToDefaultScrollingDuration(a.getInt(attr, DEFAULT_SWIPING_TO_REFRESH_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshLayout_release_to_refreshing_scrolling_duration) {
                    setReleaseToRefreshingScrollingDuration(a.getInt(attr, DEFAULT_RELEASE_TO_REFRESHING_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshLayout_refresh_complete_delay_duration) {
                    setRefreshCompleteDelayDuration(a.getInt(attr, DEFAULT_REFRESH_COMPLETE_DELAY_DURATION));

                } else if (attr == R.styleable.SwipeRefreshLayout_refresh_complete_to_default_scrolling_duration) {
                    setRefreshCompleteToDefaultScrollingDuration(a.getInt(attr, DEFAULT_REFRESH_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshLayout_default_to_refreshing_scrolling_duration) {
                    setDefaultToRefreshingScrollingDuration(a.getInt(attr, DEFAULT_DEFAULT_TO_REFRESHING_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshLayout_swiping_to_load_more_to_default_scrolling_duration) {
                    setSwipingToLoadMoreToDefaultScrollingDuration(a.getInt(attr, DEFAULT_SWIPING_TO_LOAD_MORE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshLayout_release_to_loading_more_scrolling_duration) {
                    setReleaseToLoadingMoreScrollingDuration(a.getInt(attr, DEFAULT_RELEASE_TO_LOADING_MORE_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshLayout_load_more_complete_delay_duration) {
                    setLoadMoreCompleteDelayDuration(a.getInt(attr, DEFAULT_LOAD_MORE_COMPLETE_DELAY_DURATION));

                } else if (attr == R.styleable.SwipeRefreshLayout_load_more_complete_to_default_scrolling_duration) {
                    setLoadMoreCompleteToDefaultScrollingDuration(a.getInt(attr, DEFAULT_LOAD_MORE_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshLayout_default_to_loading_more_scrolling_duration) {
    ...
    protected void onFinishInflate() {
        ...
        if (mHeaderView != null && mHeaderView instanceof Refreshable) {
            mHeaderView.setVisibility(GONE);
        }
        if (mFooterView != null && mFooterView instanceof Refreshable) {
    ...
    /**
     * set refresh header view, the view must at lease be an implement of {@code PullRefreshable}.
     * the view can also implement {@code Refreshable} for more extension functions
     *
     * @param view
     */
    public void setRefreshHeaderView(View view) {
        if (view instanceof PullRefreshable) {
            ...
            Log.e(TAG, "Refresh header view must be an implement of PullRefreshable");
        }
    }
    ...
    public void setLoadMoreFooterView(View view) {
        if (view instanceof PushRefreshable) {
    ...
    /**
     * set an {@link OnPullRefreshListener} to listening refresh event
     *
     * @param listener
     */
    ...
    /**
     * set an {@link OnPushRefreshListener} to listening load more event
     *
     * @param listener
     */
    public void setOnPushRefreshListener(OnPushRefreshListener listener) {
        this.mPushRefreshListener = listener;
    }
    ...
    RefreshCallback mRefreshCallback = new RefreshCallback() {
        ...
        public void onPrepare() {
            if (mHeaderView != null && mHeaderView instanceof Refreshable && STATUS.isStatusDefault(mStatus)) {
                mHeaderView.setVisibility(VISIBLE);
                ((Refreshable) mHeaderView).onPrepare();
            }
        }

        @Override
        public void onMove(int y, boolean isComplete, boolean automatic) {
            if (mHeaderView != null && mHeaderView instanceof Refreshable && STATUS.isRefreshStatus(mStatus)) {
                if (mHeaderView.getVisibility() != VISIBLE) {
                    mHeaderView.setVisibility(VISIBLE);
                }
                ((Refreshable) mHeaderView).onMove(y, isComplete, automatic);
            }
        }

        @Override
        public void onRelease() {
            if (mHeaderView != null && mHeaderView instanceof Refreshable && STATUS.isReleaseToRefresh(mStatus)) {
                ((Refreshable) mHeaderView).onRelease();
            }
        }
        ...
        public void onRefresh() {
            if (mHeaderView != null && STATUS.isRefreshing(mStatus)) {
                if (mHeaderView instanceof PullRefreshable) {
                    ((PullRefreshable) mHeaderView).onRefresh();
                }
        ...
        public void onComplete() {
            if (mHeaderView != null && mHeaderView instanceof Refreshable) {
                ((Refreshable) mHeaderView).onComplete();
            }
        }
        ...
        public void onReset() {
            if (mHeaderView != null && mHeaderView instanceof Refreshable && STATUS.isStatusDefault(mStatus)) {
                ((Refreshable) mHeaderView).onReset();
                mHeaderView.setVisibility(GONE);
            }
        }
    };
    ...
    LoadMoreCallback mLoadMoreCallback = new LoadMoreCallback() {
        ...
        @Override
        public void onPrepare() {
            if (mFooterView != null && mFooterView instanceof Refreshable && STATUS.isStatusDefault(mStatus)) {
                mFooterView.setVisibility(VISIBLE);
                ((Refreshable) mFooterView).onPrepare();
            }
        }

        @Override
        public void onMove(int y, boolean isComplete, boolean automatic) {
            if (mFooterView != null && mFooterView instanceof Refreshable && STATUS.isLoadMoreStatus(mStatus)) {
                if (mFooterView.getVisibility() != VISIBLE) {
                    mFooterView.setVisibility(VISIBLE);
                }
                ((Refreshable) mFooterView).onMove(y, isComplete, automatic);
            }
        }

        @Override
        public void onRelease() {
            if (mFooterView != null && mFooterView instanceof Refreshable && STATUS.isReleaseToLoadMore(mStatus)) {
                ((Refreshable) mFooterView).onRelease();
            }
        }
        ...
        public void onLoadMore() {
            if (mFooterView != null && STATUS.isLoadingMore(mStatus)) {
                if (mFooterView instanceof PushRefreshable) {
                    ((PushRefreshable) mFooterView).onLoadMore();
                }
                if (mPushRefreshListener != null) {
                    mPushRefreshListener.onPushRefresh();
                }
            }
        }
        ...
        public void onComplete() {
            if (mFooterView != null && mFooterView instanceof Refreshable) {
                ((Refreshable) mFooterView).onComplete();
            }
        }
        ...
        public void onReset() {
            if (mFooterView != null && mFooterView instanceof Refreshable && STATUS.isStatusDefault(mStatus)) {
                ((Refreshable) mFooterView).onReset();
                mFooterView.setVisibility(GONE);
            }
        }
    };
     ...
     */
    abstract class RefreshCallback implements Refreshable, PullRefreshable {
    }

    /**
     * load more event callback
     */
    abstract class LoadMoreCallback implements Refreshable, PushRefreshable {
    }
    ...
    private class AutoScroller implements Runnable {
        ...
        public void run() {
            ...
            if (finish) {
                ...
                mmLastY = currY;
                SwipeRefreshLayout.this.autoScroll(yDiff);
                post(this);
            }
        }
    ...
    public static interface OnPushRefreshListener {
        void onPushRefresh();
    }
